...
comment on column OFFICE_WORK_HOLIDAY.REG_USER_ID is '등록자ID';
comment on column OFFICE_WORK_HOLIDAY.MOD_USER_ID is '수정자ID';
...
comment on column OFFICE_BATCH_HISTORY.ERROR_MESSAGE is '에러메시지';
comment on column OFFICE_BATCH_HISTORY.SUCCESS_YN is '성공여부(Y/N)';
...
(
  ...
  RESULT_MOD_YN CHAR(1) DEFAULT 'N',
...
comment on column OFFICE_COMMUTE_DAY.RESULT_MOD_YN is '근태결과수정여부';
...
(
  ...
  TARDY_YN CHAR(1) DEFAULT 'N',
  HOLIDAY_YN CHAR(1) DEFAULT 'N',
  COMMUTE_SUBMIT_STATUS_CODE VARCHAR(50),
...
comment on column OFFICE_COMMUTE_DEPT_DAY.HOLIDAY_YN is '공휴일여부';
...
comment on column OFFICE_COMMUTE_DEPT_DAY.WORKED_TIME_VALUE is '평균근무시간';
...
comment on column OFFICE_WORK_REPORT.HOLIDAY_YN is '공휴일 여부';
...
(
  ...
  PAST_OCCUR_COUNT NUMERIC(5, 1) NOT NULL DEFAULT 0,
  PAST_USE_COUNT NUMERIC(5, 1) NOT NULL DEFAULT 0,
  REG_DATE TIMESTAMP DEFAULT NOW(),
  MOD_DATE TIMESTAMP DEFAULT NOW(),
  REG_USER_ID VARCHAR(50),
  MOD_USER_ID VARCHAR(50)
...
comment on column OFFICE_VACATION_YEAR.PAST_OCCUR_COUNT is '과년월차(발생)';
comment on column OFFICE_VACATION_YEAR.PAST_USE_COUNT is '과년월차(사용)';
...
comment on column OFFICE_VACATION_DETAIL_DAY_HISTORY.VACATION_COUNT is '휴가 count : 반차인 경우 0.5';
...
comment on column OFFICE_VACATION_YEAR_PREVIEW.PAST_OCCUR_COUNT is '과년월차(발생)';
comment on column OFFICE_VACATION_YEAR_PREVIEW.PAST_USE_COUNT is '과년월차(사용)';
...
(
  PREVIEW_ID VARCHAR(50) PRIMARY KEY,
  USER_ID VARCHAR(50) NOT NULL,
  BASE_YEAR VARCHAR(4) NOT NULL,
  VACATION_NAME VARCHAR(2000),
  VACATION_COUNT NUMERIC(5, 1) NOT NULL DEFAULT 0,
